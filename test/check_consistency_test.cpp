...
namespace sgm
{
    ...
    static void check_consistency_(HostImage &dispL, const HostImage &dispR, const HostImage &srcL, bool subpixel, int LR_max_diff)
    {
        ...
        for (int y = 0; y < h; y++)
        {
            ...
            for (int x = 0; x < w; x++)
            {
                ...
                const int k = x - d;
                if (mask == 0 || disp == sgm::INVALID_DISP || (k >= 0 && k < w && LR_max_diff >= 0 && abs(ptrDispR[k] - d) > LR_max_diff))
                {
