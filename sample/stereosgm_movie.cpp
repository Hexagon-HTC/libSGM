...
static const std::string keys = "{ @left-image-format  | <none> | format string for path to input left image  }"
...
int main(int argc, char *argv[])
{
    ...
    ASSERT_MSG(disp_size == 64 || disp_size == 128 || disp_size == 256 || disp_size == 512, "disparity size must be 64, 128, 256 or 512.");
    ...
    for (int frame_no = start_number;; frame_no++)
    {
        ...
        cv::putText(disparity_color, cv::format("sgm execution time: %4.1f[msec] %4.1f[FPS]", 1e-3 * duration, fps), cv::Point(50, 50), 2, 0.75, cv::Scalar(255, 255, 255));
