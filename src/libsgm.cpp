...
namespace sgm
{
    ...
    static bool has_enough_depth(int dst_depth, int disparity_size, int min_disp, bool subpixel)
    {
        ...
        if (min_disp <= 0)
        {
            ...
            if (min < -(1ll << (dst_depth - 1)) || 1ll << (dst_depth - 1) <= max)
                return false;
        }
    ...
    {
    public:
        Impl(int width, int height, int disparity_size, int src_depth, int dst_depth, int src_pitch, int dst_pitch, ExecuteInOut inout_type, const Parameters &param) :
            width_(width),
            height_(height),
    ...
    StereoSGM::StereoSGM(int width, int height, int disparity_size, int src_depth, int dst_depth, ExecuteInOut inout_type, const Parameters &param)
    {
        impl_ = new Impl(width, height, disparity_size, src_depth, dst_depth, width, width, inout_type, param);
    }

    StereoSGM::StereoSGM(int width, int height, int disparity_size, int src_depth, int dst_depth, int src_pitch, int dst_pitch, ExecuteInOut inout_type, const Parameters &param)
    {
