...
// This file contains disparity 512 cost aggregation kernels with DP_BLOCK_SIZE=16.
// 512 disparity requires SUBGROUP_SIZE=32, which mandates DP_BLOCK_SIZE=16.
// Compiled separately to allow 64/128/256 to use optimal DP_BLOCK_SIZE=8.
...
#define SHFL_UP_512(mask, var, delta, w) __shfl_up_sync((mask), (var), (delta), (w))
#define SHFL_DOWN_512(mask, var, delta, w) __shfl_down_sync((mask), (var), (delta), (w))
#else
#define SHFL_UP_512(mask, var, delta, width) __shfl_up((var), (delta), (width))
#define SHFL_DOWN_512(mask, var, delta, width) __shfl_down((var), (delta), (width))
#endif
...
namespace sgm
{
    ...
    namespace cost_aggregation_512
    {
        // DP_BLOCK_SIZE=16 is required for 512 disparity (SUBGROUP_SIZE=32)
        ...
        template<unsigned int SUBGROUP_SIZE>
        struct DynamicProgramming
        {
            ...
            __device__ void update(uint32_t *local_costs, uint32_t p1, uint32_t p2, uint32_t mask)
            {
                ...
                uint32_t lazy_out = 0, local_min = 0;
                {
                    ...
                    const uint32_t prev = SHFL_UP_512(mask, dp[DP_BLOCK_SIZE - 1], 1, WARP_SIZE);
                ...
                }
                {
                    ...
                    const uint32_t next = SHFL_DOWN_512(mask, dp0, 1, WARP_SIZE);
                    uint32_t out = min(dp[k] - last_min, p2);
                    out = min(out, dp[k - 1] - last_min + p1);
        ...
        namespace vertical
        {
            ...
            template<typename CENSUS_TYPE, int DIRECTION>
            __global__ void aggregate_vertical_path_kernel_512(uint8_t *dest, const CENSUS_TYPE *left, const CENSUS_TYPE *right, int width, int height, unsigned int p1,
                                                               unsigned int p2, int min_disp)
            {
                static const unsigned int MAX_DISPARITY = 512;
                static const unsigned int SUBGROUP_SIZE = MAX_DISPARITY / DP_BLOCK_SIZE; // 32
                static const unsigned int PATHS_PER_WARP = WARP_SIZE / SUBGROUP_SIZE;    // 1
                static const unsigned int PATHS_PER_BLOCK = BLOCK_SIZE / SUBGROUP_SIZE;  // 8
                ...
                DynamicProgramming<SUBGROUP_SIZE> dp;
            ...
            void aggregate_up2down_512(COST_TYPE *dest, const CENSUS_TYPE *left, const CENSUS_TYPE *right, int width, int height, unsigned int p1, unsigned int p2, int min_disp,
                                       cudaStream_t stream)
            {
                static const unsigned int MAX_DISPARITY = 512;
                ...
                aggregate_vertical_path_kernel_512<CENSUS_TYPE, 1><<<gdim, bdim, 0, stream>>>(dest, left, right, width, height, p1, p2, min_disp);
                CUDA_CHECK(cudaGetLastError());
            }

            template<typename CENSUS_TYPE>
            void aggregate_down2up_512(COST_TYPE *dest, const CENSUS_TYPE *left, const CENSUS_TYPE *right, int width, int height, unsigned int p1, unsigned int p2, int min_disp,
                                       cudaStream_t stream)
            {
                static const unsigned int MAX_DISPARITY = 512;
                static const unsigned int SUBGROUP_SIZE = MAX_DISPARITY / DP_BLOCK_SIZE;
        ...
        namespace horizontal
        {
            ...
            template<typename CENSUS_TYPE, int DIRECTION>
            __global__ void aggregate_horizontal_path_kernel_512(uint8_t *dest, const CENSUS_TYPE *left, const CENSUS_TYPE *right, int width, int height, unsigned int p1,
                                                                 unsigned int p2, int min_disp)
            {
                static const unsigned int MAX_DISPARITY = 512;
                static const unsigned int SUBGROUP_SIZE = MAX_DISPARITY / DP_BLOCK_SIZE;
                ...
                DynamicProgramming<SUBGROUP_SIZE> dp[DP_BLOCKS_PER_THREAD];
                    ...
                    for (unsigned int i = 0; i < DP_BLOCK_SIZE; ++i)
                    {
                        ...
                        for (unsigned int j = 0; j < DP_BLOCKS_PER_THREAD; ++j)
                        {
                            ...
                            if (DIRECTION > 0)
                            {
                                ...
                                right_buffer[j][0] = SHFL_UP_512(shfl_mask, t, 1, SUBGROUP_SIZE);
                                if (lane_id == 0)
                                {
                            ...
                            else
                            {
                                ...
                                right_buffer[j][DP_BLOCK_SIZE - 1] = SHFL_DOWN_512(shfl_mask, t, 1, SUBGROUP_SIZE);
                                if (lane_id + 1 == SUBGROUP_SIZE)
                                {
            ...
            template<typename CENSUS_TYPE>
            void aggregate_left2right_512(COST_TYPE *dest, const CENSUS_TYPE *left, const CENSUS_TYPE *right, int width, int height, unsigned int p1, unsigned int p2, int min_disp,
                                          cudaStream_t stream)
            {
                static const unsigned int MAX_DISPARITY = 512;
                static const unsigned int SUBGROUP_SIZE = MAX_DISPARITY / DP_BLOCK_SIZE;
                ...
                const int bdim = BLOCK_SIZE;
                aggregate_horizontal_path_kernel_512<CENSUS_TYPE, 1><<<gdim, bdim, 0, stream>>>(dest, left, right, width, height, p1, p2, min_disp);
                CUDA_CHECK(cudaGetLastError());
            }

            template<typename CENSUS_TYPE>
            void aggregate_right2left_512(COST_TYPE *dest, const CENSUS_TYPE *left, const CENSUS_TYPE *right, int width, int height, unsigned int p1, unsigned int p2, int min_disp,
                                          cudaStream_t stream)
            {
                static const unsigned int MAX_DISPARITY = 512;
                static const unsigned int SUBGROUP_SIZE = MAX_DISPARITY / DP_BLOCK_SIZE;
        ...
        namespace oblique
        {
            ...
            template<typename CENSUS_TYPE, int X_DIRECTION, int Y_DIRECTION>
            __global__ void aggregate_oblique_path_kernel_512(uint8_t *dest, const CENSUS_TYPE *left, const CENSUS_TYPE *right, int width, int height, unsigned int p1,
                                                              unsigned int p2, int min_disp)
            {
                static const unsigned int MAX_DISPARITY = 512;
                static const unsigned int SUBGROUP_SIZE = MAX_DISPARITY / DP_BLOCK_SIZE;
            ...
            template<typename CENSUS_TYPE>
            void aggregate_upleft2downright_512(COST_TYPE *dest, const CENSUS_TYPE *left, const CENSUS_TYPE *right, int width, int height, unsigned int p1, unsigned int p2,
                                                int min_disp, cudaStream_t stream)
            {
                ...
                const int bdim = BLOCK_SIZE;
                aggregate_oblique_path_kernel_512<CENSUS_TYPE, 1, 1><<<gdim, bdim, 0, stream>>>(dest, left, right, width, height, p1, p2, min_disp);
                CUDA_CHECK(cudaGetLastError());
            }

            template<typename CENSUS_TYPE>
            void aggregate_upright2downleft_512(COST_TYPE *dest, const CENSUS_TYPE *left, const CENSUS_TYPE *right, int width, int height, unsigned int p1, unsigned int p2,
                                                int min_disp, cudaStream_t stream)
            {
                static const unsigned int MAX_DISPARITY = 512;
                static const unsigned int SUBGROUP_SIZE = MAX_DISPARITY / DP_BLOCK_SIZE;
            ...
            template<typename CENSUS_TYPE>
            void aggregate_downright2upleft_512(COST_TYPE *dest, const CENSUS_TYPE *left, const CENSUS_TYPE *right, int width, int height, unsigned int p1, unsigned int p2,
                                                int min_disp, cudaStream_t stream)
            {
                static const unsigned int MAX_DISPARITY = 512;
                static const unsigned int SUBGROUP_SIZE = MAX_DISPARITY / DP_BLOCK_SIZE;
            ...
            template<typename CENSUS_TYPE>
            void aggregate_downleft2upright_512(COST_TYPE *dest, const CENSUS_TYPE *left, const CENSUS_TYPE *right, int width, int height, unsigned int p1, unsigned int p2,
                                                int min_disp, cudaStream_t stream)
            {
                static const unsigned int MAX_DISPARITY = 512;
                static const unsigned int SUBGROUP_SIZE = MAX_DISPARITY / DP_BLOCK_SIZE;
    ...
    namespace details
    {
        ...
        template<typename CENSUS_TYPE>
        void cost_aggregation_512_impl(const DeviceImage &srcL, const DeviceImage &srcR, DeviceImage &dst, int P1, int P2, PathType path_type, int min_disp)
        {
            ...
            cost_aggregation_512::vertical::aggregate_up2down_512<CENSUS_TYPE>(dst.ptr<COST_TYPE>(0), left, right, width, height, P1, P2, min_disp, streams[0]);
            cost_aggregation_512::vertical::aggregate_down2up_512<CENSUS_TYPE>(dst.ptr<COST_TYPE>(1), left, right, width, height, P1, P2, min_disp, streams[1]);
            cost_aggregation_512::horizontal::aggregate_left2right_512<CENSUS_TYPE>(dst.ptr<COST_TYPE>(2), left, right, width, height, P1, P2, min_disp, streams[2]);
            cost_aggregation_512::horizontal::aggregate_right2left_512<CENSUS_TYPE>(dst.ptr<COST_TYPE>(3), left, right, width, height, P1, P2, min_disp, streams[3]);

            if (path_type == PathType::SCAN_8PATH)
            {
                cost_aggregation_512::oblique::aggregate_upleft2downright_512<CENSUS_TYPE>(dst.ptr<COST_TYPE>(4), left, right, width, height, P1, P2, min_disp, streams[4]);
                cost_aggregation_512::oblique::aggregate_upright2downleft_512<CENSUS_TYPE>(dst.ptr<COST_TYPE>(5), left, right, width, height, P1, P2, min_disp, streams[5]);
                cost_aggregation_512::oblique::aggregate_downright2upleft_512<CENSUS_TYPE>(dst.ptr<COST_TYPE>(6), left, right, width, height, P1, P2, min_disp, streams[6]);
                cost_aggregation_512::oblique::aggregate_downleft2upright_512<CENSUS_TYPE>(dst.ptr<COST_TYPE>(7), left, right, width, height, P1, P2, min_disp, streams[7]);
            }
        ...
        {
            cost_aggregation_512_impl<uint32_t>(srcL, srcR, dst, P1, P2, path_type, min_disp);
        }
        ...
        {
            cost_aggregation_512_impl<uint64_t>(srcL, srcR, dst, P1, P2, path_type, min_disp);
        }
